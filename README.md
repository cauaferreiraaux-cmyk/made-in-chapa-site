...
- **Dado que não existe não é inventado.** Campo `null` não vira placeholder na tela:
  simplesmente não é desenhado. Preços e descrições são os publicados pela própria
  casa no seu canal de pedidos; item cujo valor a loja não publica aparece sem preço.
